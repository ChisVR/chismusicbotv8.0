...
var streambot = require('./streambot/streambot');
...
BOTS: ${chalk.magenta('MUSIC')}, ${chalk.cyan('COMMANDS')}, ${chalk.green('STREAMBOT')}
${chalk.grey('--------------------------------------------------')}
`);



const { TOKEN, PREFIX } = require("./commandbot/util/BotUtil");

const { ShardingManager } = require('discord.js');

const manager = new ShardingManager('./commandbot/commandbot.js', {
    totalShards: 1,
    token:  TOKEN
});
...
manager.on('shardCreate', (shard) => {
    ...
    return Promise.all(promises).then((results) => {
      ...
      fetch(`https://botrix.cc/api/v1/bot/`, {
        method: "post",
